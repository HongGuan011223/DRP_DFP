...
def ctrsbox(xopt, g, H, projections, delta, d_max_iters=100, d_tol=1e-10, use_fortran=USE_FORTRAN):
    ...
    for ii in range(MAX_LOOP_ITERS):
        # Compute projected gradient descent step

        # Don't forget to update the gradient

        # update CRVMIN - leave this
        # will need to calculate value of s above, i.e. s = step from x to new x
        crv = s.dot(H).dot(s)/sumsq(s) if sumsq(s) >= ZERO_THRESH else crvmin
        crvmin = min(crvmin, crv) if crvmin != -1.0 else crv

        # exit condition, we will need a stopping condition to exit loop

    # return step s.t. x + d is feasible, new gradient, and crvmin
...
def ctrsbox_linear(xbase, g, projections, Delta, d_max_iters=100, d_tol=1e-10, use_fortran=USE_FORTRAN):
    ...
    # projected GD loop - similar to above, only need to return d this time
